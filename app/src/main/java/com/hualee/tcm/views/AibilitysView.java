    ...
    private void initSize() {
        if (allAbility == null) {
            allAbility = new AbilityRenderInfo[]{
                    ...
                    new AbilityRenderInfo("测试标题4", "69"),
                    new AbilityRenderInfo("测试标题5", "57"),
                    new AbilityRenderInfo("测试标题6", "87"),
                    new AbilityRenderInfo("测试标题6", "77"),
                    new AbilityRenderInfo("测试标题8", "66"),
                    new AbilityRenderInfo("测试标题9", "87"),
            };
            n = allAbility.length;//十条辺
        } else {
            n = allAbility.length;
        }
...
//        setMeasuredDimension(viewWidth, viewHight);
    ...
    private void drawAbilityLine(Canvas canvas) {
        ...
        for (int i = 0; i < n; i++) {
            float r = R * (Float.valueOf(allAbility[i].getScore()) / 100.0f);  //能力值/100再乘以半径就是所占的比例
            float x = (float) (r * Math.cos(i * angle - Math.PI / 2));
            float y = (float) (r * Math.sin(i * angle - Math.PI / 2));
     ...
     * @param data
     */
    public void setData(AbilityRenderInfo[] data) {
        if (data == null) {
            return;
        }
